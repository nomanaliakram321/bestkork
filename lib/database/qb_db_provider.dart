...
class QuestionDbProvider {
  ...
  static final QuestionDbProvider db = QuestionDbProvider();
  ...
  /// search items Using by language
  Future<List<Questions>> questionAndAnswerSearch(String lang) async {
    final db = await database;
    var result =
        await db.query('Question', where: 'lang = ?', whereArgs: [lang]);
    List<Questions> list = result.isNotEmpty
        ? result.map((c) => Questions.fromJson(c)).toList()
        : [];
    print(list);
